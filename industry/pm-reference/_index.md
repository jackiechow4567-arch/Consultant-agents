# BESREMi PM reference library (distilled)

> **For agents:** When role lens = PM, read this index then load only the files relevant to the question.  
> **Sensitivity:** Public/regulatory/clinical publications = Mid/Low. Internal transcripts and draft dossiers = see `work-briefs/pm/README.md` (High — local only).  
> **Last built:** 2026-07-27 from user's Perplexity Space file set (Downloads).

## Load order

1. [`../role-pm.md`](../role-pm.md) — decision criteria, Do/Don't, output format (always)
2. Pick one or more reference files below by topic

## Reference files

| File | Use when |
|------|----------|
| [besremi-hk-pi-summary.md](besremi-hk-pi-summary.md) | Indication, dosing, contraindications, PI-bound claims, IFN potency warning |
| [pv-clinical-evidence.md](pv-clinical-evidence.md) | PROUD-PV, Low-PV, comparative stats vs HU/phlebotomy, claim boundaries |
| [et-clinical-evidence.md](et-clinical-evidence.md) | SURPASS-ET / EXCEED-ET and ET pathway papers — **indication-application only**, not HK promo |
| [et-hk-treatment-options.md](et-hk-treatment-options.md) | Current HK ET options: HA category, registration, local practice (Gill/AMWG) |
| [regulatory-compliance.md](regulatory-compliance.md) | UMAO, HKAPI CoP, promo rules, hospitality, ADR reporting |
| [ha-formulary-access.md](ha-formulary-access.md) | HADF mechanics, HA drug spend context, formulary submission logic |
| [hk-hospitals-landscape.md](hk-hospitals-landscape.md) | Key HK hospitals for field/access planning (no PII) |
| [hkapi-policy-updates.md](hkapi-policy-updates.md) | 2025 PHC sharing session, 2026 CoP TTT workshop themes |
| [local-source-index.md](local-source-index.md) | Map to original files on this machine (`Downloads/`) |
| [vendor-ma-onboarding-pack.md](vendor-ma-onboarding-pack.md) | What to share with a market-access vendor (Wave 1 PV + labelled ET indication-application folder vs hold-until-CDA); Medical training agenda |

## Prohibitions for agents

- Do not treat reference summaries as permission to invent claims — always cross-check PI + global lexicon
- Do not quote advisory-board transcript content (not in this folder)
- Do not paste internal pricing/PSP uptake into external tools
- Do not email named-patient or case-level HK experience to vendors; use the Wave 1 / Wave 2 split in `vendor-ma-onboarding-pack.md`
- Do not put ET papers in the PV (on-label) vendor folder; use the labelled Folder B in that pack and `et-clinical-evidence.md`
